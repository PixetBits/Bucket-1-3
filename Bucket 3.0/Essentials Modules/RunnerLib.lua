...
require('Essentials Modules/NumLib')
require('Essentials Modules/StrLib')
require('Essentials Modules/BolLib')
require('Essentials Modules/LstLib')
require('Essentials Modules/XLogic')
...
--# Class stuff --------------------------------------------#--
    ...
    if line:sub(1, 3) == "%[to" and line:sub(-1) == "%]" and line ~= "[to Basic]" then
        ...
        -- The file exists --
        if io.read('../Importable Libraries/' .. lname) == nil then sys.error(elin, 'c', 'This library could not be found.')
        else ilib[lname] = require('../Importable Libraries/' .. lname) end
    end
...
--# Script functions and resourses--------------------------#--
    ...
    --# Put yours lib's modules bellow ---------------------#--
    --# Replaceable values ---------------------------------#--
    ...
    --# Non user dependent functions -----------------------#--
        ...
        if line:sub(1, 4) == 'for ' then
            ...
            for n = 1, 8 do
                ...
                if door(elin, condt, actN, actS, actB, actL) then

                    for lopl = clin + 1, eidx - 1 do
                    
                        run(lines, lopl, true, name, true)
                    end
                end
            end
        end
        ...
        if line:sub(1, 6) == 'every ' then
            ...
            -- Run for every value --
                ...
                if actL[list].type == 'num' then
                    ...
                    for i in pairs(actL[list]) do

                        -- Update value --
                        actN[item] = actL[list][i]
...
--# Execution stuff ----------------------------------------#--
    ...
    -- Next line --
    if sub ~= true then clin = clin + 1 end
